...
void write_matrix(char *filename,int size,int n_nonzeros,int *edge_i,int *index_j,CPX *data,\
		  int findx)
{
    ...
    for(int i=0;i<size;i++){
	    ...
	    if(data==NULL){
	        myfile<<i+1<<" "<<index_j[j]-findx+1<<" "<<1<<endl;
	    }else{
	        myfile<<i+1<<" "<<index_j[j]-findx+1<<" "<<real(data[j])<<" "<<imag(data[j])<<endl;
	    }
	}
    }
    myfile.close();
